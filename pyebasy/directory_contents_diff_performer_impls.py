...
class DefaultContentsDifferencePerformer(DirectoryContentsDifferencePerformer):
    ...
    def do_before_subtree(self, directory_path: Path, diff: DirectoryContentsDifference, contents_supplier: FileContentsSupplier, storage_modifier: StorageModifier):
        loggr.log_detailed(f"Executing the directory contents difference of directory {directory_path} with {len(diff.changes())} changes (before subtree)...")
        ...
        loggr.log_detailed(f"Executed the directory contents difference (before subtree)!")
    ...
    def do_after_subtree(self, directory_path: Path, diff: DirectoryContentsDifference, contents_supplier: FileContentsSupplier, storage_modifier: StorageModifier):
        loggr.log_detailed(f"Executing the directory contents difference of directory {directory_path} with {len(diff.changes())} changes (after subtree) ...")
        ...
        loggr.log_detailed(f"Executed the directory contents difference (after subtree)!")
